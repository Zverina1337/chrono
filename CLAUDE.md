...
src-tauri/src/    # Rust backend (Tauri)
  lib.rs          # Builder, setup(БД), плагины (log, opener), регистрация 30 команд
  main.rs         # точка входа
  error.rs        # AppError (thiserror + JSON Serialize: { type, message })
  utils.rs        # now_utc(), next_position(), bool_from_i32()
  db/
    mod.rs        # init_db(), PRAGMA WAL + foreign_keys
    migrations.rs # SQL-миграции (user_version, текущая v2)
  ...
  commands/       # Tauri-команды (валидация + lock handling + repository)
...
## Стек Rust-бэкенда
- **БД:** SQLite через rusqlite (bundled) + миграции через user_version (текущая v2)
- **UUID:** uuid v1 (v4)
- **Время:** chrono (UTC, формат "YYYY-MM-DD HH:MM:SS")
- **Ошибки:** thiserror + JSON Serialize `{ type, message }` (варианты: Database, NotFound, Validation, Internal, LockError)
- **Логирование:** log + tauri-plugin-log
- **Состояние:** Mutex<Connection> в Tauri State (lock с map_err → AppError::LockError)
...
**Важные поля:**
- `tasks.estimated_seconds` (Option<i64>) — оценка времени в секундах
- `time_entries.duration_seconds` (Option<i64>) — длительность в секундах
- FK-индексы на всех внешних ключах (миграция v2)

## Валидация (commands слой)
- `name.trim().is_empty()` → AppError::Validation
- `name.len() > 255` → AppError::Validation
- Применяется к create/update для: projects, statuses, priorities, tasks, subtasks, labels
